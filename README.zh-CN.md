...
- 使用七套带色谱预览的配色区分蛋白质链；同一次会话不重复链颜色，同时清晰
  区分配体、核酸、糖链、离子和水。
- 保留原始默认渲染，或选择明确配置环境光遮蔽、阴影、深度提示、抗锯齿和
  透明渲染的影棚质量/发表质量预设；也可显示文件名和链标签。
- 按链、元素、氨基酸类型、二级结构、疏水性、电荷、置信度或自定义残基/原子
  标量进行配色。
- 自动读取文件已标注功能位点、检测坐标几何口袋候选，或标记链界面两侧的
  表位/互补位；可在可缩放序列矩阵和三维结构间联动选择。
- 通过触控选择测量距离、夹角、二面角、RMSD、半径和近似埋藏界面面积。
- 选择残基或原子，进行原子级聚焦，并显示所有已叠合可见结构中 5 Å 范围内的
  完整残基。
- 使用基于序列或仅基于坐标的对应关系叠合整条链或自定义区域，并提供 RMSD、
  临时重叠预览、应用和撤销。
- 通过局域网网址分享可恢复场景，录制 360° 或结构翻页视频，并向 Mac 导出
  同步视角的候选结构图库、透明 PNG 和场景元数据。
...
- **创建会话链接** 将坐标和场景状态保存到 Mac，并生成同一 Wi-Fi 可打开的
  短网址。**视频**支持指定轴 360° 旋转和多结构翻页；**候选结构图库**可固定
  当前相机以及指定受体结构或链。
...
共享会话链接可以恢复场景；普通导出 JSON 仍是结果记录。详细说明请参阅
[场景图像导出（英文）](docs/export.md)。

## 隐私与网络范围

设备本地文件在浏览器内解析。只有在明确创建会话链接时，当前坐标文本和场景
状态才会复制到 Mac 的 `shared-sessions/`。RCSB 请求由浏览器直接发送到 RCSB
官方下载服务。

Mac 文件库为只读。它只提供配置文件夹内的 PDB/mmCIF 文件，跳过隐藏文件和
符号链接，也不提供删除 API。场景导出写入生成的 PNG/JSON/视频；会话分享写入
含坐标的、以随机 ID 命名的 JSON 快照。
...
目前只加载第一个坐标模型，不展开生物学组装体或轨迹。普通刷新不会自动保存
未分享的场景；创建会话链接后可通过该网址恢复。PWA 安装和云存储不在当前版本
范围内。
