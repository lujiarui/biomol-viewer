...
`representations` creates one component per protein chain, followed by nucleic-acid, ligand, glycan, ion, lipid, and water components. Lipids are excluded from the ligand component to avoid duplication. A monotonically increasing session index assigns every protein component a distinct color; removed and reloaded colors are not recycled until Purge starts a new session. Palette changes regenerate the same unique index range while retaining component visibility. Cartoon, cartoon-with-sticks, backbone, line, ball-and-stick, space-filling, and molecular-surface modes affect protein; non-protein species retain their distinguishing representation.
...
`createViewer` uses public PluginContext APIs without any Mol* desktop panels. The primary camera-focus behavior is omitted so taps do not zoom unexpectedly. Native Mol* camera input remains. Close inspection lowers the camera distance floor and near clipping limit. Purpose-based visual presets use Mol* renderer lights and postprocessing plus cartoon geometry parameters. Optional HTML chain labels project component centers through the Mol* camera after each draw; they are pointer-transparent and excluded from viewport image exports.
