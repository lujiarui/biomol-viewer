...
The default directory is `protein-ipad/scene-exports/`. Use `PROTEIN_EXPORTS="/absolute/path/to/exports" npm run dev` (or npm run serve) to choose another directory. This is separate from the read-only Mac structure library. Restart the server after changing the environment variable.
...
The Mac-hosted development and production preview servers support export. A plain static host cannot save files on the Mac. No cloud storage is involved.
