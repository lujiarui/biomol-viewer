...
export function sceneExports(): Plugin {
  const root = path.resolve(process.env.PROTEIN_EXPORTS || 'scene-exports');
  const handler: Connect.NextHandleFunction = (req,res,next) => {
    ...
    void (async () => {
      ...
      if (req.method === 'POST' && url.pathname === '/api/exports') {
        ...
        if (req.headers['x-protein-export'] !== '1' || !req.headers['content-type']?.startsWith('application/json')) { res.statusCode=415; res.end(); return; }
