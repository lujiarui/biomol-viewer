...
test.describe('captured touch regions',()=>{
  ...
  test('capture both counterparts, preserving them while panels close and reopen',async({page})=>{
    test.setTimeout(120_000);
    await page.goto('/');await page.getByLabel('Open structure file').setInputFiles('public/examples/example.cif');await expect(page.getByRole('main')).toHaveAttribute('data-loaded','true');
...
test('Mac export rejects cross-origin writes and path traversal',async({request})=>{
  const cross=await request.post('/api/exports',{headers:{Origin:'https://unrelated.example','X-Biomol-Export':'1'},data:{image:'invalid',manifest:{}}});expect(cross.status()).toBe(403);
  const invalid=await request.post('/api/exports',{headers:{Origin:'http://localhost:5173','X-Biomol-Export':'1'},data:{image:'data:text/html;base64,PGgxPg==',manifest:{}}});expect(invalid.status()).toBe(400);
