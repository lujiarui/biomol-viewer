...
test('geometry quick mode chooses matching chain, applies immediately, supports undo and purge',async({page})=>{
 ...
 await page.getByRole('button',{name:'Close structures panel'}).click();await page.getByLabel('Open structure file').setInputFiles('public/examples/example.cif');await expect(page.getByRole('main')).toHaveAttribute('data-loaded','true');
...
test('small pinch zoom changes camera distance gradually and settles',async({page,request})=>{
 ...
 try{
  await page.goto('/');await page.getByLabel('Open structure file').setInputFiles('public/examples/example.cif');await expect(page.getByRole('main')).toHaveAttribute('data-loaded','true');await page.waitForTimeout(600);const before=await cameraDistance();
