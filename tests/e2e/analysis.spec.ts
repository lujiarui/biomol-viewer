...
test('3D residue-pair mode is explicit and the sequence matrix stays compact',async({page})=>{
 ...
 const panel=page.getByRole('complementary',{name:'Structure analysis'}),matrix=panel.getByLabel('Linked sequence'),range=panel.getByRole('button',{name:'Drag residue pair on 3D'});
 await expect(matrix).toBeVisible();expect(await matrix.evaluate(el=>getComputedStyle(el).maxHeight)).toBe('220px');
 await range.click();await expect(range).toHaveAttribute('aria-pressed','true');await expect(page.locator('.viewer-canvas')).toHaveClass(/range-selecting/);await expect(panel.getByText('Only the two endpoints are selected',{exact:false})).toBeVisible();
 await range.click();await expect(page.locator('.viewer-canvas')).not.toHaveClass(/range-selecting/);
});

test('dragging between two ribbon residues selects only the endpoint pair with a live guide',async({page})=>{
 ...
 expect(points.size).toBe(2);const [first,last]=[...points.values()];await page.getByRole('button',{name:'Analyze'}).click();await page.getByRole('button',{name:'Drag residue pair on 3D'}).click();
 await page.mouse.move(first.x,first.y);await page.mouse.down();await page.mouse.move(last.x,last.y,{steps:8});const guide=page.locator('.residue-drag-guide');await expect(guide).toBeVisible();expect(parseFloat(await guide.locator('.residue-drag-arrow').evaluate(el=>getComputedStyle(el).width))).toBeGreaterThan(20);await page.mouse.up();
 await expect(sheet).toBeVisible();await expect(sheet.locator('.eyebrow')).toHaveText('2 SELECTED RESIDUES');
});
