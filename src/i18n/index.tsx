...
export function I18nProvider({children}:{children:ReactNode}){
 const [language,setLanguage]=useState<Language>(()=>{try{const saved=localStorage.getItem('biomol-language') ?? localStorage.getItem('protein-language');if(saved==='en'||saved==='zh-CN')return saved;}catch{/* Storage can be unavailable in private sessions. */}return navigator.language.toLowerCase().startsWith('zh')?'zh-CN':'en';});
 useEffect(()=>{document.documentElement.lang=language;document.title=language==='zh-CN'?'Biomol · 分子结构查看器':'Biomol · Molecular viewer';try{localStorage.setItem('biomol-language',language);}catch{/* The switch still works without persistence. */}},[language]);
