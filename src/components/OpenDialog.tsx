...
interface Props { examplesFirst?:boolean; busy: boolean; error?: string; onClose: () => void; onDevice: () => void; onRcsb: (id: string) => void; onMac: (name: string) => void; onExample: (id: string) => void; onPair: () => void; }
export function OpenDialog({ examplesFirst,busy, error, onClose, onDevice, onRcsb, onMac, onExample, onPair }: Props) {
  ...
  useEffect(() => { dialog.current?.showModal();if(examplesFirst){gallery.current?.scrollIntoView({block:'start'});gallery.current?.focus({preventScroll:true});}void refresh(); }, []);
  return localize(<dialog ref={dialog} className="open-dialog" onCancel={e => { e.preventDefault(); if (!busy) onClose(); }}>
    ...
    <section aria-label="Mac library"><div className="library-heading"><h3>Mac library</h3><button disabled={refreshing || busy} onClick={() => void refresh()}>Refresh</button></div>
      ...
      <ul className="library-list">{files.map(file => <li key={file.name}><button disabled={busy} onClick={() => onMac(file.name)}><span>{file.name}</span><small>{Math.max(1, Math.round(file.size / 1024))} KB · {new Date(file.modified).toLocaleString(language)}</small></button></li>)}</ul>
    </section>
