...
export function App() {
  ...
  return localize(<main className={`app${holding?' preview-held':''}`} aria-label="Biomol viewer" data-ready={ready} data-loaded={scene.length > 0} aria-busy={state.loading}>
    ...
    {scene.length > 0 && !state.selection.residues.length && <footer className="gesture-hint">{rangeSelecting ? 'Drag between ribbon residues' : 'Drag to rotate'} <span>·</span> Pinch to zoom <span>·</span> {additive ? 'Tap to add/remove residues' : `Tap a ${picking}`} </footer>}
